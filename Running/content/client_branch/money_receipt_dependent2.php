...
if ($mr_type == "Freight Bill") {
    ...
    $Query1 = "select stationary_no,order_no from sr_bilty where order_no='" . $order_no . "'";
    $UDB1->query($Query1);
    while ($UDB1->Multicoloums()) {
        $stationary_no = $UDB1->Record["stationary_no"];
    }
    $Query = "select distinct(no_item),bill_date_item,pending_amount from $tablename where no_item='" . $stationary_no . "' and pending_amount>0";
    $UDB->query($Query);
    while ($UDB->Multicoloums()) {
        $str = $str . "\"" . $UDB->Record["no_item"] . "\"" . ",";
    }
