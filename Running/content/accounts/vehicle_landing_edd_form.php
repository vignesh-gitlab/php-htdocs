...
$actionpage = 'vehicle_landing_edd_action.php';
...
if ($orderno_error == false) {
    $Query = "SELECT  order_no,client_name,client_division,client_branch,orgin,destination,vehicle_type,vehicle_no,dispatch_date,dispatch_time,expected_dateof_delivery from sr_vehicle_dispatch where order_no='" . $_REQUEST["order_no"] . "'";
    $UDB->query($Query);
...
if ($id_error == false) {
    $Query = "SELECT  id,order_no,client_name,client_division,client_branch,orgin,destination,vehicle_type,vehicle_no,dispatch_date,dispatch_time,expected_dateof_delivery,landing_date,landing_time from $tablename where id='" . $_REQUEST["id"] . "'";
    $UDB->query($Query);
    while ($UDB->Multicoloums()) {
        $order_no = $UDB->Record["order_no"];
        $client_name = $UDB->Record["client_name"];
                        ...
                        <div class="box-body table-responsive">
                            <div class="form_tablebox">
                                <table cellspacing="0">
                                    <tr>
                                        <td  class="form_label_split2">
                                            <span class="red">*&nbsp;</span>Delay Reason
                                        </td>
                                        <td class="form_content_split2" align="center">
                                            <input type="text" required="true" name="delay_reason" class="form-control" value="<?php if ($id_error == false) echo $delay_reason; ?>">
                                        </td>
                                    </tr>
                                </table>
                            </div>
