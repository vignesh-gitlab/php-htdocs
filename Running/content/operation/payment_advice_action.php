...
if ($_REQUEST["form_action"] == "Insert") {
    ...
    for ($i = 1; $i <= $_REQUEST["product_count"]; $i++) {
        if ((isset($_REQUEST["lr_no" . $i]) && !empty($_REQUEST["lr_no" . $i]))) {
            $Query = "insert into $tablename1 values(NULL,'" . $_REQUEST["order_no"] . "','" . $_REQUEST["lr_no" . $i] . "','" . $_REQUEST["challan_no" . $i] . "','" . $_REQUEST["lr_date" . $i] . "','" . $_REQUEST["un_ld_date" . $i] . "','" . $_REQUEST["bal_amt" . $i] . "','" . $_REQUEST["hamali_paid" . $i] . "','" . $_REQUEST["hamali_collect" . $i] . "','" . $_REQUEST["tds_amt" . $i] . "','" . $_REQUEST["claim_amt" . $i] . "','" . $_REQUEST["late_delivery" . $i] . "','" . $_REQUEST["destination" . $i] . "','" . $_REQUEST["line_total" . $i] . "')";
            $UDB->query($Query);
        }
    }
