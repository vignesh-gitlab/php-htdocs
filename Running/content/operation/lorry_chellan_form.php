...
if ($orderno_error == false) {
    ...
    $Query1 = "SELECT lane_from,lane_to,branch_code,branch_city,vehicle_no,delivery_address_line1,delivery_address_line2,delivery_city,delivery_pincode,packing from sr_bilty where order_no='" . $_REQUEST["order_no"] . "'";
    $UDB1->query($Query1);
                                    ...
                                    for ($i = 1; $i <= 10; $i++) {
                                        ...
                                        <tr>
                                            <td class="form_content_multiple">
                                                <input type="text" name="code<?php echo $i; ?>"  id="code<?php echo $i; ?>" class="form-control" value="<?php if ($id_error == false) echo $code_array[$i - 1]; ?>">
                                            </td>
                                      <!--  <select class="form-control dropdown_padding" name="code<?php echo $i; ?>" id="code<?php echo $i; ?>" onchange="AjaxFunction_display_consignment(this.value,<?php echo $i; ?>)">
                                            <option>Select</option>
                                            <?php /*  $Query = "select distinct(product_brand) from sr_product where product_group='" . $product_group . "' order by product_name";
                                              $DB->query($Query);

                                              while ($DB->Multicoloums()) {
                                              if ($id_error == false && $product_category_array[$i - 1] == $DB->Record["product_brand"]) {
                                            ...
                                            <td class="form_content_multiple">
                                                <input type="text" name="consignment_no<?php echo $i; ?>"  id="consignment_no<?php echo $i; ?>" class="form-control" value="<?php if ($id_error == false) echo $consignment_no_array[$i - 1]; ?>">
                                            </td>
                                            <td class="form_content_multiple">
                                                <input type="text"  id="no_of_packages<?php echo $i; ?>" name="no_of_packages<?php echo $i; ?>" class="form-control" value="<?php
                                                if ($id_error == false)
                                                    echo $no_of_packages_array[$i - 1];
                                                ?>">
                                            </td>
                                            <td class="form_content_multiple">
                                                <input type="text" id="packing<?php echo $i; ?>" name="packing<?php echo $i; ?>" class="form-control" value="<?php
                                                if ($id_error == false)
