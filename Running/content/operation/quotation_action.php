...
$return_page = 'quotation_grid.php';
...
if ($_REQUEST["form_action"] == "Insert") {
    ...
    $Query = "insert into $tablename values(NULL,'" . $new_max_id . "','" . $new_max_orderno . "','" . $_REQUEST["quotation_date"] . "','" . $_REQUEST["description"] . "','" . $_REQUEST["vehicle_required_date"] . "','" . $_REQUEST["vehicle_type"] . "','" . $_REQUEST["client_name"] . "','" . $_REQUEST["division_name"] . "','" . $_REQUEST["branch_name"] . "','" . $_REQUEST["address_line1"] . "','" . $_REQUEST["address_line2"] . "','" . $_REQUEST["city"] . "','" . $_REQUEST["pincode"] . "','" . $_REQUEST["contact_no"] . "','" . $_REQUEST["tax_category"] . "','','" . $_REQUEST["terms_and_condition"] . "','" . $_REQUEST["sub_total"] . "','" . $_REQUEST["sale_tax"] . "','" . $_REQUEST["discount"] . "','" . $_REQUEST["grand_total"] . "','" . $_SESSION["username"] . "','','Open')";
    $UDB->query($Query);

    for ($i = 1; $i <= $_REQUEST["product_count"]; $i++) {
        if ($_REQUEST["type_of_movement" . $i] != "Select" || (isset($_REQUEST["lane_from" . $i]) && !empty($_REQUEST["lane_from" . $i]))) {
            $Query = "insert into $tablename1 values(NULL,'" . $new_max_orderno . "','" . $_REQUEST["vehicle_type"] . "','" . $_REQUEST["type_of_movement" . $i] . "','" . $_REQUEST["charge_base" . $i] . "','" . $_REQUEST["lane_id" . $i] . "','" . $_REQUEST["lane_from" . $i] . "','" . $_REQUEST["lane_to" . $i] . "','" . $_REQUEST["line_total" . $i] . "','" . $_REQUEST["ex_total" . $i] . "','" . $_REQUEST["tax_rate" . $i] . "','" . $_REQUEST["tax_type" . $i] . "','" . $_REQUEST["line_tax" . $i] . "','" . $_REQUEST["amount" . $i] . "')";
            $UDB->query($Query);
        }
    }
    ...
    $Query = "insert into $tablename values(NULL,'" . $_REQUEST["quotation_id"] . "','" . $_REQUEST["quotation_no"] . "','" . $_REQUEST["quotation_date"] . "','" . $_REQUEST["description"] . "','" . $_REQUEST["vehicle_required_date"] . "','" . $_REQUEST["vehicle_type"] . "','" . $_REQUEST["client_name"] . "','" . $_REQUEST["division_name"] . "','" . $_REQUEST["branch_name"] . "','" . $_REQUEST["address_line1"] . "','" . $_REQUEST["address_line2"] . "','" . $_REQUEST["city"] . "','" . $_REQUEST["pincode"] . "','" . $_REQUEST["contact_no"] . "','" . $_REQUEST["tax_category"] . "','','" . $_REQUEST["terms_and_condition"] . "','" . $_REQUEST["sub_total"] . "','" . $_REQUEST["sale_tax"] . "','" . $_REQUEST["discount"] . "','" . $_REQUEST["grand_total"] . "','" . $_SESSION["username"] . "','','Open')";
    $UDB->query($Query);
    for ($i = 1; $i <= $_REQUEST["product_count"]; $i++) {
        if ($_REQUEST["type_of_movement" . $i] != "Select" || (isset($_REQUEST["lane_from" . $i]) && !empty($_REQUEST["lane_from" . $i]))) {
            $Query = "insert into $tablename1 values(NULL,'" . $_REQUEST["quotation_no"] . "','" . $_REQUEST["vehicle_type"] . "','" . $_REQUEST["type_of_movement" . $i] . "','" . $_REQUEST["charge_base" . $i] . "','" . $_REQUEST["lane_id" . $i] . "','" . $_REQUEST["lane_from" . $i] . "','" . $_REQUEST["lane_to" . $i] . "','" . $_REQUEST["line_total" . $i] . "','" . $_REQUEST["ex_total" . $i] . "','" . $_REQUEST["tax_rate" . $i] . "','" . $_REQUEST["tax_type" . $i] . "','" . $_REQUEST["line_tax" . $i] . "','" . $_REQUEST["amount" . $i] . "')";
            $UDB->query($Query);
        }
    }
} else if ($_REQUEST["approval"] == "Approve") {

    $Query = "update $tablename set quotation_status='Approval',dota_no='" . $_REQUEST["dota_no"] . "',approved_by='" . $_SESSION["username"] . "' where id=" . $_REQUEST["id"];
    $UDB->query($Query);
