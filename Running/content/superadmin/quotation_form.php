...
$leadno_error = false;
if ((!isset($_REQUEST["lead_no"])) || (empty($_REQUEST["lead_no"]))) {
    $leadno_error = true;
}
?>
...
<script type="text/javascript">
    ...
    function AjaxFunction_display_type_movement(client_name)
    {
        ...
        function stateck()
        {
            ...
            if (httpxml.readyState == 4)
            {
                ...
                {
                    var myarray = eval(httpxml.responseText);
                    var length = document.getElementById("type_of_movement" + line_id).options.length;
                    for (j = (length - 1); j >= 0; j--)
                    {
                        document.getElementById("type_of_movement" + line_id).remove(j);
                    }
                    ...
                    if (myarray.length > 0)
                    {
                        ...
                        optn.text = "Select";
                        optn.value = "Select";
                        document.getElementById("type_of_movement" + line_id).options.add(optn);
                        for (i = 0; i < myarray.length; i++)
                        {
                            var optn = document.createElement("OPTION");
                            optn.text = myarray[i];
        ...
        var url = "quotation_dependent7.php";
        url = url + "?client_name=" + client_name;
        //url=url+"&sid="+Math.random();
        httpxml.onreadystatechange = stateck;
        httpxml.open("GET", url, true);
    ...
    function AjaxFunction_display_charge_base(type_of_movement, line_id)
    {
        ...
        function stateck()
        {
            ...
            {
                var myarray = eval(httpxml.responseText);
                var length = document.getElementById("charge_base" + line_id).options.length;
                for (j = (length - 1); j >= 0; j--)
                {
                    document.getElementById("charge_base" + line_id).remove(j);
                }
                ...
                if (myarray.length > 0)
                {
                    ...
                    optn.text = "Select";
                    optn.value = "Select";
                    document.getElementById("charge_base" + line_id).options.add(optn);
                    for (i = 0; i < myarray.length; i++)
                    {
                        var optn = document.createElement("OPTION");
                        optn.text = myarray[i];
        ...
        var url = "quotation_dependent5.php";
        //var vehicle_type = encodeURIComponent(vehicle_type);
        url = url + "?type_of_movement=" + type_of_movement;
        //url=url+"&sid="+Math.random();
        httpxml.onreadystatechange = stateck;
        httpxml.open("GET", url, true);
        httpxml.send(null);
    }
    function AjaxFunction_display_lane_value(charge_base, line_id)
    {
        var httpxml;
        try
        {
        ...
        var url = "quotation_dependent6.php";
        //var vehicle_type = encodeURIComponent(vehicle_type);
        url = url + "?charge_base=" + charge_base;
        //url=url+"&sid="+Math.random();
        httpxml.onreadystatechange = stateck;
        httpxml.open("GET", url, true);
        httpxml.send(null);
    ...
    {
        var type_of_movement = document.getElementById('type_of_movement' + line_id).value;
        var charge_base = document.getElementById('charge_base' + line_id).value;
        var httpxml;
        try
        {
            // Firefox, Opera 8.0+, Safari
        ...
        function stateck()
        {
            ...
            if (httpxml.readyState == 4)
            {
                ...
                if (myarray.length > 0)
                {
                    ...
                    var tax_category = document.getElementById('tax_category').value;
                    if (tax_category == "Non-Taxable") {
                        document.getElementById('amount' + line_id).value = myarray[2];
                        calc();
                    }
                }
            }
        ...
        url = url + "?type_of_movement=" + type_of_movement + "&charge_base=" + charge_base + "&lane_id=" + lane_id;
        //url=url+"&sid="+Math.random();
        httpxml.onreadystatechange = stateck;
        httpxml.open("GET", url, true);
        httpxml.send(null);
    ...
    function calc()
    {
        var sub_total = 0;
        for (i = 1; i <= 25; i++) {
            sub_total = Number(document.getElementById('amount' + i).value) + sub_total
        }
    ...
    function AjaxFunction_display_tax(tax_category)
    {
        if (tax_category == "Non-Taxable") {
            for (i = 1; i <= 25; i++) {
                document.getElementById('tax_rate' + i).disabled = true;
                document.getElementById('tax_type' + i).disabled = true;
                document.getElementById('line_tax' + i).readOnly = true;
            }
        } else if (tax_category == "Taxable") {
            for (i = 1; i <= 25; i++) {
                document.getElementById('tax_rate' + i).disabled = false;
                document.getElementById('tax_type' + i).disabled = false;
                document.getElementById('line_tax' + i).readOnly = true;
            }
        }
    ...
    function AjaxFunction_display_terms(terms_list)
    {
        ...
        } else
        {
            ...
            var url = "quotation_dependent4.php";
    ...
    /* function display_amount(line_total, line_id)
     {
     var line_total = document.getElementById('line_total' + line_id).value;
     document.getElementById('amount' + line_id).value = line_total;
     }*/
    function transfer_value(line_id)
    {
        var unit_total = document.getElementById('line_total' + line_id).value;
        document.getElementById('ex_total' + line_id).value = unit_total;
        calculate_total(line_id);
    }
    ...
    {         //var unit_total = document.getElementById('line_total' + line_id).value;
        ...
        var ex_total = document.getElementById('ex_total' + line_id).value;
        var line_amount = 0;
        var total_amount = 0;
        var tax_amount = 0;
        if (tax_rate != "NA")
        {
            if (tax_type == "Ex")
            {
                tax_amount = ((Number(ex_total) * Number(tax_rate)) / 100);
            }
            if (tax_type == "In")
            {                 //tax_amount = (Number(tax_rate) / 100) * Number(unit_total);
                tax_amount = (Number(ex_total) - (Number(ex_total) * 100) / (Number(tax_rate) + 100));
            }
        }
        if (tax_type == "Ex")
        {
            line_amount = (Number(ex_total));
            total_amount = (Number(ex_total) + Number(tax_amount));
        }
        if (tax_type == "In")
        {
            //line_amount = (Number(unit_total) - Number(tax_amount));
            line_amount = (Number(ex_total) * 100) / (Number(tax_rate) + 100);
            total_amount = (Number(ex_total));
        }
    ...
    function AjaxFunction_validate_remarks(line_total, line_id)
    {
        var client_name = document.getElementById('client_name').value;
        var type_of_movement = document.getElementById('type_of_movement' + line_id).value;
        var charge_base = document.getElementById('charge_base' + line_id).value;
        var lane_id = document.getElementById('lane_id' + line_id).value;
        var httpxml;
        try
        {
            // Firefox, Opera 8.0+, Safari
        ...
        function stateck()
        {
            if (httpxml.readyState == 4)
            {
                ...
                {
                    alert(myarray[0]);
                    document.getElementById('remarks' + line_id).value = "";
                }
            }
        }
        var url = "quotation_dependent8.php";
        url = url + "?client_name=" + client_name + "&type_of_movement=" + type_of_movement + "&charge_base=" + charge_base + "&lane_id=" + lane_id + "&line_total=" + line_total;
        //url=url+"&sid="+Math.random();
        httpxml.onreadystatechange = stateck;
        httpxml.open("GET", url, true);
        httpxml.send(null);
...
<?php
if ($id_error == false) {
    $Query = "SELECT quotation_id,quotation_no,quotation_date,description,vehicle_required_date,vehicle_type,client_name,division_name,branch_name,address_line1,address_line2,city,pincode,contact_no,tax_category,dota_no,terms_and_condition,sub_total,total_tax,discount,grand_total from $tablename where id='" . $_REQUEST["id"] . "'";
    $UDB->query($Query);
    ...
    $Query = "SELECT  id,type_of_movement,charge_base,lane_id,lane_from,lane_to,unit_price,ex_total,tax_rate,tax_type,tax_total,amount from $tablename1 where quotation_no='" . $quotation_no . "'";
    $UDB->query($Query);
    while ($UDB->Multicoloums()) {
        $edit_product_count = $edit_product_count + 1;
        $type_of_movement_array[] = $UDB->Record["type_of_movement"];
        $charge_base_array[] = $UDB->Record["charge_base"];
...
if ($form_error == false) {
    $Query = "SELECT  id,quotation_id from $tablename where id='" . $_REQUEST["id"] . "'";
    $UDB->query($Query);
    while ($UDB->Multicoloums()) {
        $quotation_id = $UDB->Record["quotation_id"];
    }
}
if ($quotation_error == false) {
    $Query = "SELECT  id,quotation_no from $tablename where id='" . $_REQUEST["id"] . "'";
    $UDB->query($Query);
    while ($UDB->Multicoloums()) {
        $quotation_no = $UDB->Record["quotation_no"];
    }
...
<aside class="right-side strech">
    ...
    <section class="content">
        <div class="row">
            <div class="col-xs-12">
                <form role="form" name="form" id="form1" method="post" onsubmit="return checkForm(t
                                his);" action="<?php echo $actionpage; ?>" enctype="multipart/form-data" autocomplete="<?php echo AUTOCOMPLETE ?>">
                    <div class="box">
                                ...
                                <table cellspacing="0">
                                    <tr>
                                        ...
                                        <td class="form_content_split2" align="left">
                                            <select class="chosen-select form-control dropdown_padding" name="client_name" id="client_name" onchange="AjaxFunction_display_type_movement(this.value)" onchange="AjaxFunction_display_division_name(this.value)">
                                        ...
                                        <td class="form_content_split2">
                                            ...
                                            ?>
                                            <input type="text" name="quotation_no" readonly class="form-control" value="<?php
                                            if ($form_error == false && $id_error == false) {
                                                $Query = "SELECT  id,count(quotation_no) as quotation_count,max(quotation_no)as max_quotation_no from $tablename where quotation_no like'" . $quotation_no . "%'";
                                                $UDB->query($Query);
                                                while ($UDB->Multicoloums()) {
                                                    $quotation_count = $UDB->Record["quotation_count"];
                                                    $max_quotation_no = $UDB->Record["max_quotation_no"];
                                                }
                                                $quotation_no_split = explode('.', $max_quotation_no);
                                                $quotation_no_count = count($quotation_no_split);
                                                if ($quotation_no_count == 1) {
                                                    $quotation_no = $quotation_no . ".1";
                                                } else {
                                                    $quotation_no = $quotation_no_split[0] . "." . ($quotation_no_split[1] + 1);
                                                }
                                                echo $quotation_no;
                                            ...
                                            ?>">
                                                   ...
                                                   if ($form_error == false) {
                                                       ?>
                                                <input type="hidden" name="quotation_id" class="form-control" value="<?php echo $quotation_id; ?>">
                                                <?php
                                            }
                                            ?>
                                        </td>
                                    ...
                                    <tr>
                                        ...
                                        <td class="form_content_split2" align="center">
                                            <div class="input-group">
                                                <input type="text" readonly="true" name="quotation_date" class="form-control pull-right" id="quotation_date" onfocus="pick_date(this
                                                                .id);" value="<?php
                                                       if ($id_error == false) {
                                                           echo $quotation_date;
                                                       } else {
                                    ...
                                    <tr>
                                        ...
                                        <td class="form_content_split2" rowspan="3" align="center">
                                            <textarea name="description" style="height:78px;" class="form-control" autofocus><?php if ($id_error == false) echo $description; ?></textarea>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td  class="form_label_split2" rowspan="3">
                                    ...
                                    <tr>
                                        ...
                                        <td class="form_content_split2" align="center">
                                            <div class="input-group">
                                                <input type="text" readonly="true" name="vehicle_required_date" class="form-control pull-right" id="vehicle_required_date" onfocus="pick_date(this
                                                                        .id);" value="<?php
                                                       if ($id_error == false) {
                                    ...
                                    <tr>
                                        ...
                                        <td class="form_content_split2" align="center">
                                            <select class="form-control dropdown_padding" name="tax_category" id="tax_category" onchange="AjaxFunction_display_tax(this.value)">
                                                <option>Select</option>
                                                <?php
                                                $Query = "select distinct(tax_category) from master_tax_category order by tax_category";
                                                $DB->query($Query);

                                                while ($DB->Multicoloums()) {
                                                    if ($id_error == false && $tax_category == $DB->Record["tax_category"]) {
                                                        echo'<option selected>' . $DB->Record["tax_category"] . '</option>';
                                                    } else {
                                                        echo'<option>' . $DB->Record["tax_category"] . '</option>';
                                                    }
                                                }
                                                ?>
                                            </select>
                                        ...
                                        <td class="form_content_split2" align="center">
                                            <?php if ($approval_error == false) { ?>
                                                <input type="text" name="dota_no" class="form-control" value="<?php if ($id_error == false) echo $dota_no; ?>">
                                            <?php } else { ?>
                                                <input type="text" id="contact_number" readonly name="dota_no" class="form-control" value="<?php if ($id_error == false) echo $dota_no; ?>">
                                            <?php } ?>
                                        </td>
                                    </tr>
                                </table>
                        ...
                        <div class="box-body table-responsive">
                            <div class="form_tablebox">
                                <table cellspacing="0">
                                    ...
                                    for ($i = 1; $i <= 25; $i++) {
                                        ...
                                        <tr>
                                            <td class="form_content_multiple">
                                                <select class="form-control dropdown_padding" name="type_of_movement<?php echo $i; ?>" id="type_of_movement<?php echo $i; ?>" onchange="AjaxFunction_display_charge_base(this.value,<?php echo $i; ?>)" >
                                                    <option>Select</option>
                                                    <?php
                                                    if ($id_error == false && $type_of_movement_array[$i - 1] != NULL) {
                                                        echo'<option selected>' . $type_of_movement_array[$i - 1] . '</option>';
                                                    }
                                                    ?>
                                                </select>
                                            </td>
                                            <td class="form_content_multiple">
                                                <select class="form-control dropdown_padding" name="charge_base<?php echo $i; ?>" id="charge_base<?php echo $i; ?>" onchange=" AjaxFunction_display_lane_value(this.value,<?php echo $i; ?>)">
                                                    <option>Select</option>
                                                    <?php
                                                    if ($id_error == false && $charge_base_array[$i - 1]) {
                                                        echo'<option selected>' . $charge_base_array[$i - 1] . '</option>';
                                                    } else {
                                                        echo'<option selected>' . $charge_base_array[$i - 1] . '</option>';
                                                    }
                                                    ?>
                                            ...
                                            <td class="form_content_multiple">
                                                <select class="form-control dropdown_padding" name="lane_id<?php echo $i; ?>" id="lane_id<?php echo $i; ?>" onchange="AjaxFunction_display_lane_detail(this.value,<?php echo $i; ?>)">
                                                    <option>Select</option>
                                                    <?php
                                                    if ($id_error == false && $lane_id_array[$i - 1]) {
                                            ...
                                            <td class="form_content_multiple">
                                                <input type="text" style="text-align:right; float:left;" id="line_total<?php echo $i; ?>"  name="line_total<?php echo $i; ?>" onblur="transfer_value(<?php echo $i; ?>); AjaxFunction_validate_remarks(this.value,<?php echo $i; ?>);" class="form-control" value="<?php
                                                if ($id_error == false) {
                                            ...
                                            <td class="form_content_multiple">
                                                <?php if ($tax_category == "Non-Taxable") { ?>
                                                    <select disabled="disabled"  class="form-control dropdown_padding" style="width:32%; float:left;" onchange="calculate_total(<?php echo $i; ?>)" id="tax_rate<?php echo $i; ?>" name="tax_rate<?php echo $i; ?>">
                                                        <option>NA</option>
                                                        <?php
                                                    ...
                                                    <select disabled="disabled" class="form-control dropdown_padding" style="width:27%;  float:left;" onchange="calculate_total(<?php echo $i; ?>)" id="tax_type<?php echo $i; ?>" name="tax_type<?php echo $i; ?>">
                                                        <?php
                                                    ...
                                                    <input type="text" style="text-align:right; width:41%;"  id="line_tax<?php echo $i; ?>" readonly name="line_tax<?php echo $i; ?>" class="form-control" value="<?php
                                                    if ($id_error == false) {
                                                    ...
                                                    <select  class="form-control dropdown_padding" style="width:32%; float:left;" onchange="calculate_total(<?php echo $i; ?>)" id="tax_rate<?php echo $i; ?>" name="tax_rate<?php echo $i; ?>">
                                                        <option>NA</option>
                                                        <?php
                                                        $Query = "select distinct(tax_rate) from sr_tax_rate order by tax_rate";
                                                        $DB->query($Query);
                                            ...
                                            <td class="form_content_multiple">
                                                <input type="text" style="text-align:right;" readonly id="amount<?php echo $i; ?>"  name="amount<?php echo $i; ?>" onclick="display_amount(<?php echo $i; ?>)" onblur="AjaxFunction_validate_remarks(this.value,<?php echo $i; ?>)" class="form-control" value="<?php
                                                if ($id_error == false) {
                                            ...
                                            <td class="form_content_multiple">
                                                <input type="text" style="text-align:left;"  id="remarks<?php echo $i; ?>"  name="remarks<?php echo $i; ?>"  class="form-control" value="<?php
                                                if ($id_error == false) {
                                                    echo $remarks_array[$i - 1];
                                                }
                                                ?>">
                                            </td>
                                        </tr>
                                    ...
                                    ?>
                                    <tr>
                                        <td colspan="6" class="form_label_multiple">Terms & Condition</td>
                                        <td colspan="2" class="form_label_multiple_right">Subtotal</td>
                                    ...
                                    <tr>
                                        <td colspan="6" rowspan="3" class="form_content_multiple">
                                        ...
                                        <td colspan="2" class="form_label_multiple_right">(+) Total Tax</td>
                                        <td colspan="1" class="form_content_multiple"><input type="text" style="text-align:right;" readonly id="sale_tax" name="sale_tax" class="form-control" value="<?php
                                            if ($id_error == false) {
                                                echo $total_tax;
                                            } else {
                                                echo"0.00";
                                    ...
                                    <tr>
                                        <td colspan="2" class="form_label_multiple_right">Grand Total</td>
                    ...
                    <div class="box-footer">
                        ...
                        if ($quotation_error != false) {
                            ...
                            <button type="submit" onsubmit="this.style.display = 'none';
                                            ...
                                            ajax_load.style.display = 'bl
                                            ock';" class="btn-sm btn-primary"><i class="fa fa-fw fa-check-square-o"></i>&nbsp;Submit</button>
                                    <?php
                                }
                                ?>
                                <?php
                            ...
                            <button type="submit" onsubmit="this.style.display = 'none';
                                                ...
                                                submit_loader.style.display = 'block';
                                                ajax_load.style.display = 'bl
                                                        ock';" class="btn-sm btn-primary"><i class="fa fa-fw fa-check-square-o"></i>&nbsp;Approve</button>
                                    <?php
                                }
                                ?>
                        ...
                        <?php
                        if ($id_error == false && $quotation_error != false) {
                            if ($form_error == false) {
                                echo'<input type="hidden" name="form_action" value="Duplicate"/>';
                            } else {
                                echo'<input type="hidden" name="form_action" value="Update"/>';
                                echo'<input type="hidden" name="id" value="' . $_REQUEST["id"] . '"/>';
                            }
                        } else if ($quotation_error == false) {
