...
$Query1 = "SELECT id,branch_code,company_name from sr_company where company_type='Head Office'";
$DB1->query($Query1);
while ($DB1->Multicoloums()) {
    $corporate_branch_code = $DB1->Record["branch_code"];
    $corporate_branch_name = $DB1->Record["company_name"];
}

if ($id_error == false) {
    $Query = "SELECT  id,branch_id,branch_code,zonal_code,zonal_branch_name,company_name,company_caption,address_line1,address_line2,city,pincode,telephone_no,mobile_no,fax_no,email_id,website_id,tin_no,cst_no from $tablename where id='" . $_REQUEST["id"] . "'";
    $DB->query($Query);
    while ($DB->Multicoloums()) {
        $branch_id = $DB->Record["branch_id"];
        $branch_code = $DB->Record["branch_code"];
        $zonal_code = $DB->Record["zonal_code"];
        $zonal_branch_name = $DB->Record["zonal_branch_name"];
...
<script type="text/javascript">
    function AjaxFunction_display_zone_branch_name(zonal_code)
    {
        ...
        function stateck()
        {
            ...
            if (httpxml.readyState == 4)
            {
                ...
                if (myarray.length > 0)
                {
                    document.form.zonal_branch_name.value = myarray[0];

                }
            }
        }
        var url = "zone_branch_dependent1.php"
        var zonal_code = encodeURIComponent(zonal_code);
        url = url + "?zonal_code=" + zonal_code;
...
<aside class="right-side strech">
    ...
    <section class="content-header">
        ...
        <ol class="breadcrumb">
            ...
            <li><a href="branch_grid.php">Branch</a></li>
                                ...
                                <table cellspacing="0">
                                    <tr>
                                        ...
                                        <td class="form_content_split2" align="center">
                                            <input type="text" readonly name="corporate_branch_code" class="form-control" value="<?php
                                            if ($id_error == false)
                                                echo $corporate_branch_code;
                                            else
                                                echo $corporate_branch_code;
                                            ?>">
                                        </td>
                                        ...
                                        <td class="form_content_split2" align="center">
                                            <input type="text" readonly name="corporate_branch_name" class="form-control" value="<?php
                                            if ($id_error == false)
                                                echo $corporate_branch_name;
                                            else
                                                echo $corporate_branch_name;
                                            ?>">
                                        </td>
                                    ...
                                    <tr>
                                        ...
                                        <td class="form_content_split2" align="left">
                                            <select class="form-control dropdown_padding" name="zonal_code" id="zonal_code" onclick="AjaxFunction_display_zone_branch_name(this.value)">
                                                <option>Select</option>
                                                <?php
                                                $Query = "select distinct(zonal_code) from sr_company where company_type='Zonal Office' order by zonal_code";
                                                $DB->query($Query);

                                                while ($DB->Multicoloums()) {
                                                    if ($id_error == false && $zonal_code == $DB->Record["zonal_code"]) {
                                                        echo'<option selected>' . $DB->Record["zonal_code"] . '</option>';
                                                    } else {
                                                        echo'<option>' . $DB->Record["zonal_code"] . '</option>';
                                                    }
                                                }
                                                ?>
                                            </select>
                                        ...
                                        <td class="form_content_split2" align="center">
                                            <input type="text" readonly name="zonal_branch_name" id="zonal_branch_name" class="form-control" value="<?php
                                            if ($id_error == false)
                                                echo $zonal_branch_name;
                                            else
                                                echo $zonal_branch_name;
                                            ?>">
                                        </td>
