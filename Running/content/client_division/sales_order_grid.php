...
$return_page = '../client_division/sales_order_grid.php';
?>
                ...
                <div class="box">
                    <div class="box-body table-responsive">
                        <table id="example" class="table table-bordered table-striped">
                            ...
                            <tbody>
                                <?php
                                $Query = "SELECT id,so_id,so_no,so_date,quotation_ref_no,vehicle_type,vehicle_required_date,client_name,city,so_status from $tablename where so_status<>'Close'  and client_name='" . $Display_Name . "' and (division_name='" . $_SESSION["division"] . "' or division_name='" . $_SESSION["division1"] . "' or division_name='" . $_SESSION["division2"] . "') order by abs(so_no)";
                                $UDB->query($Query);
                                while ($UDB->Multicoloums()) {
                                    $so_status = $UDB->Record["so_status"];
                                    ?>
                                    <tr>
                                        <td class="no-print">
                                            ...
                                            <?php
                                            if ($so_status != "Approval") {
                                                ?>
                                                <a href="<?php echo $form_page . "?so_no=" . $UDB->Record["so_no"] . "&approval=Approve" . "&id=" . $UDB->Record["id"]; ?>" title="Approve"><i class="fa fa-fw fa-check"></i></a>
                                                    <?php
                                                }
                                                ?>
